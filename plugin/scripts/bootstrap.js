#!/usr/bin/env node
// SessionStart bootstrap: installer + validator. Single pure-Node script:
// no install.sh, no npm deps, no unzip/tar binaries. Installs the
// ouroboros binary (dev path, local Homebrew, or GitHub release) and
// validates the plugin's own files are on disk, repairing the binary
// when it's missing/broken.
//
...
//   - CLAUDE_PLUGIN_DATA                             -> bin/ouroboros, .version
//   - OUROBOROS_DEV_BINARY                           -> local dev binary override
//
// Release archives are uniformly tar.gz across all platforms (matching
// .goreleaser.yml). Extraction only needs zlib (gunzipSync) — an in-house
...
const REPO = 'dangernoodle-io/ouroboros';
const USER_AGENT = 'ouroboros-bootstrap';
...
// os/arch naming. All platforms ship tar.gz (see .goreleaser.yml). Returns
...
function detectPlatform() {
  ...
  return { os: goos, arch: goarch, ext: 'tar.gz' };
}
...
function findLocalBinary() {
  for (const candidate of ['/usr/local/bin/ouroboros', '/opt/homebrew/bin/ouroboros']) {
...
async function installFromRelease(pluginData, get) {
  ...
  try {
    ...
    entry = extractTarEntry(tarBuf, 'ouroboros');
    if (!entry) throw new Error(`'ouroboros' entry not found in ${archiveName}`);
  } catch (err) {
...
// bootstrap runs validation and, if the binary is missing/broken, installs
...
async function bootstrap(env, opts) {
  env = env || process.env;

...
BINARY="${BINARY_DIR}/ouroboros"
VERSION_FILE="${CLAUDE_PLUGIN_DATA}/.version"
REPO="dangernoodle-io/ouroboros"

# Dev mode: use a local binary instead of fetching from GitHub.
if [ -n "${OUROBOROS_DEV_BINARY:-}" ]; then
  if [ ! -x "$OUROBOROS_DEV_BINARY" ]; then
    echo "ouroboros: dev binary not found: $OUROBOROS_DEV_BINARY" >&2
    exit 1
  fi
  mkdir -p "$BINARY_DIR"
  ...
  echo "ouroboros: installed dev binary from $OUROBOROS_DEV_BINARY"
  exit 0
fi

# Local binary: check well-known paths before hitting GitHub.
LOCAL_BIN=""
for candidate in /usr/local/bin/ouroboros /opt/homebrew/bin/ouroboros; do
  if [ -x "$candidate" ]; then
    LOCAL_BIN="$candidate"
    break
  fi
...
if [ -n "$LOCAL_BIN" ]; then
  ...
  echo "ouroboros: installed ${LOCAL_VERSION} from ${LOCAL_BIN}"
  exit 0
fi
...
case "$(uname -s)" in
  ...
  Linux)  OS="linux" ;;
  *)
    echo "ouroboros: unsupported OS: $(uname -s)" >&2
    exit 1
    ;;
esac
...
case "$(uname -m)" in
  ...
  *)
    echo "ouroboros: unsupported arch: $(uname -m)" >&2
    exit 1
    ;;
esac
...
if [ -z "$LATEST_TAG" ]; then
  echo "ouroboros: failed to fetch latest release tag" >&2
  [ -x "$BINARY" ] && exit 0
  exit 1
fi
...
echo "ouroboros: installing ${LATEST_VERSION} (${OS}/${ARCH})..."
...
ARCHIVE_NAME="ouroboros_${LATEST_VERSION}_${OS}_${ARCH}.${EXT}"
CHECKSUM_NAME="ouroboros_${LATEST_VERSION}_SHA256SUMS"
...
# Install binary.
install -m 755 "${WORK_DIR}/extracted/ouroboros" "$BINARY"
[ "$OS" = "darwin" ] && codesign -s - "$BINARY" 2>/dev/null
printf '%s' "$LATEST_VERSION" > "$VERSION_FILE"

echo "ouroboros: installed ${LATEST_VERSION}"

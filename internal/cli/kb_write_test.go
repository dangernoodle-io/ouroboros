...
func TestRunPutFlagsCreate(t *testing.T) {
	...
	summaries, err := store.QueryDocuments(db, []string{"decision"}, []string{"acme-corp"}, nil, "", nil, 10)
	require.NoError(t, err)
...
func TestRunPutStdinProjectFallback(t *testing.T) {
	...
	summaries, err := store.QueryDocuments(db, []string{"note"}, []string{"default-project"}, nil, "", nil, 10)
	require.NoError(t, err)
	assert.Len(t, summaries, 1)
}
...
func TestRunPutStdinValidationFailureNoPartialWrites(t *testing.T) {
	...
	summaries, err := store.QueryDocuments(db, nil, []string{}, nil, "", nil, 10)
	require.NoError(t, err)

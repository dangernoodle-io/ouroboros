...
CLI-only ops (run `ouroboros <cmd> --help`): `project` (create/get/list/rename/delete), `plan` (create/get/list/update), `config` (get/set), `kb delete <id>...` (multiple ids, all-or-nothing), `export`, `import`, `roadmap` (show/add/update/move/reorder/done/remove/seed), `link`/`unlink` (edges), `dashboard` (segment/refresh). Browse with `ls items`, `ls kb`, `ls plans`, `ls projects`, `ls edges`.
